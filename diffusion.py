...
import numpy as np  # <-- (NEW) Import numpy for DDIM schedule

# --- (Unchanged) FOCAL LOSS FUNCTION ---
...
class DiscreteDiffusion:
    def __init__(self, timesteps=1000, beta_start=0.0001, beta_end=0.02, num_classes=2, focal_loss_gamma=2.0, focal_loss_alpha=0.95):
    ...
    def p_sample(self, model, x_t, t_tensor, condition_image=None):
        """ Sample x_{t-1} from the model's prediction of x_0. (DDPM) """
        model.eval()
    ...
    def sample(self, model, image_size, batch_size=1, channels=1, condition_image=None):
        """ Generates new images by running the full reverse diffusion process. (DDPM) """
        ...
        for i in tqdm(reversed(range(self.num_timesteps)), desc="Sampling (DDPM)", total=self.num_timesteps, leave=False):
    ...
    @torch.no_grad()
    def p_sample_ddim(self, model, x_t, t, prev_t, condition_image=None):
        """
        A single DDIM sampling step. The core idea for discrete DDIM is to use the
        model's prediction of x0 and then use the forward process q(x_{t-1}|x0)
        to deterministically arrive at x_{t-1}.
        """
        model.eval()

        # 1. Predict x0 from the model
        if condition_image is not None:
            predicted_x0_logits = model(x_t, t, condition_image)
        else:
            predicted_x0_logits = model(x_t, t)
        ...
        # 2. If we are at the last step (prev_t is -1), the result is the model's prediction of x0
        if prev_t[0] < 0:
            return predicted_x0.float()

        # 3. Otherwise, use the forward process to sample x_{prev_t} from the predicted x0.
        # This is the deterministic step q(x_{prev_t} | x_0), which we can get
        # by reusing our q_sample function.
        x_prev = self.q_sample(predicted_x0, prev_t)
        return x_prev.float()

    @torch.no_grad()
    def sample_ddim(self, model, image_size, batch_size=1, channels=1, condition_image=None, num_inference_steps=50):
        """
        Generates new images using the faster DDIM sampling schedule.
        """
        device = next(model.parameters()).device
        img = torch.randint(0, self.num_classes, (batch_size, channels, image_size, image_size), device=device).float()
        ...
        # Create the DDIM timestep schedule
        # It skips timesteps, making the process faster
        step_ratio = self.num_timesteps // num_inference_steps
        timesteps_seq = (np.arange(0, num_inference_steps) * step_ratio).round()[::-1].copy().astype(np.int64)
        timesteps_seq_next = np.concatenate([timesteps_seq[1:], np.array([-1])]) # Next step is -1 for the last one
        
        for i in tqdm(range(num_inference_steps), desc=f"Sampling (DDIM, {num_inference_steps} steps)", total=num_inference_steps, leave=False):
            t = torch.full((batch_size,), timesteps_seq[i], device=device, dtype=torch.long)
            prev_t = torch.full((batch_size,), timesteps_seq_next[i], device=device, dtype=torch.long)
            
            img = self.p_sample_ddim(model, img, t, prev_t, condition_image)
